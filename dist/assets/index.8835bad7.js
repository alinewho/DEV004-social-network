...
function authStateChangedEvent(cb2) {
  onAuthStateChanged(auth, (user) => cb2(user));
}
const signOutUser = () => {
  signOut(auth);
...
const coleccPublic = Ja(db, "publicaciones");
const qOrdered = ol(coleccPublic, dl("creacion", "desc"));
const ROUTES = {};
...
const welcome = () => {
  ...
  <div class="button-container">
    <div class="logo-container">
      <img class="imageLogo" src="./Imagenes/C\xE1o_logo_Welcome_white.png" alt="logocactus">
    </div><br>
    <button class="bw" id="buttonLogin"> INICIAR SESI\xD3N  </button> <br>
    <button id="buttonGoogle" class="googleButton bw"> INICIAR SESI\xD3N CON GOOGLE </button><br>
...
const register = () => {
  ...
  root.innerHTML = `
    <section id="sectionOfContentRegister" class="button-container"> 
      ...
      <img class="imageLogo" src="./Imagenes/C\xE1o_logo_Welcome_white.png" alt="logocactusR"> <br> 
      </div>
      <p id="statusText"></p>
      ...
      <input id="passwordInput" class="inputNames" type="password" placeholder="Contrase\xF1a"><br>

      
      <h6 id="conditionOfRegister" class="greenText">Al crear tu cuenta, estar\xE1s aceptando los <br> <spam class="highlightedText" >t\xE9rminos y condiciones</spam> de C\xE1o</h6><br>
...
const login = () => {
  ...
  <section id="sectionOfLogin">
    ...
    <form id="loginForm">
      <div id="loginDiv">
        <p id="statusText"></p>
        <h2 class="login-title">Iniciar Sesi\xF3n</h2>
        <div id = "userDiv">
          <input id="inputUser" type="email" placeholder= "Correo Electr\xF3nico"></input> <br>
          <input id="inputPassword" type="password" placeholder= "Contrase\xF1a"></input>
        </div>
        <div id="buttons">
...
const posts = () => {
  ...
  root.innerHTML = `
    <section id= "containerMain">
        ...
        <div id="partOfwelcome">
            <img src="../Imagenes/c\xE1o_navbarwhite.png">
  ...
  async function like(idPublicacion, correoUsuario) {
    const docRef = Za(db, "publicaciones", idPublicacion);
    await Yl(docRef, { likes: _f(correoUsuario) });
  }
  async function disLike(idPublicacion, correoUsuario) {
    const docRef = Za(db, "publicaciones", idPublicacion);
    await Yl(docRef, { likes: wf(correoUsuario) });
  }
  tf(qOrdered, (querySnapshot) => {
    const postContainer = document.getElementById("post");
    postContainer.innerHTML = "";
    querySnapshot.forEach((post2) => {
      postContainer.innerHTML += `
      <div id="historyOfPosts" class="historyOfPosts">
        <div class="informationOfUser" id="${post2.id}">
              <div class="user-post">
                <img src="./Imagenes/usersinfondo.png" class="inLine">
                <div class="post-informacion">
                  <h3>${post2.data().autor}</h3>
                  <h6>${post2.data().creacion}</h6>
                </div>
              </div>
            <p class="mostrado" id="p-${post2.id}">${post2.data().descripcion}</p>
            <textarea class="oculto" id="textarea-${post2.id}">${post2.data().descripcion}</textarea>
            <div class="editright likeleft" id="buttonLike">
                <button class="like-button l-button" data-email="${post2.id}" id="like-button" ></button>
              </div>
        </div>
         ${post2.data().autor === userData().email ? `
            <div id="buttonsOfConfiguration">
              <div class="likeleft" id="likeleft">
                <button class="edit-button" data-id="${post2.id}"></button>
                <button class="delete-button" data-id="${post2.id}"></button>
              </div>
      ...
      `;
      const likeB = document.querySelectorAll(".l-button");
      likeB.forEach((btn) => {
        btn.addEventListener("click", () => {
          if (post2.data().likes.includes(userData().email)) {
            console.log("if");
    ...
    editButton.forEach((boton) => {
      boton.addEventListener("click", (e) => {
       ...
       `;
        document.getElementById(`p-${e.target.dataset.id}`).setAttribute("class", "oculto");
        const textAIn = document.getElementById(`textarea-${e.target.dataset.id}`);
        textAIn.setAttribute("class", "mostrado");
        console.log(e.target.dataset.id);
        document.getElementById(`g-${e.target.dataset.id}`).addEventListener("click", () => {
          console.log("click guardar");
          const docRef = Za(db, "publicaciones", e.target.dataset.id);
          Yl(docRef, {
    ...
    deleteButton.forEach((boton) => {
      boton.addEventListener("click", (e) => {
        ...
        if (confirm("\xBFDeseas eliminar este comentario?")) {
          const docRef = Za(db, "publicaciones", e.target.dataset.id);
          Zl(docRef);
        }
      });
